...
export class PokedexService {
  ...
  httpOptions = {
    //adiciona o cabeçalho necessário para salvar as alterações no HTTP
  ...
  getpokemon(name: any) {
    return this._http
      ...
      .pipe(tap(), catchError(this.handleError('getPokemon', [])));
  }

  getdatabyurl(url: string) {
    return this._http
      .get<{}>(url)
      .pipe(tap(), catchError(this.handleError('getPokemonSpecies', [])));
  }

  getgeneratios() {
    return this._http.get<{}>(`https://pokeapi.co/api/v2/generation`).pipe(
  ...
  getgen8() {
    return this._http
      .get<{}>(`https://pokeapi.co/api/v2/pokemon?limit=96&offset=809`)
      .pipe(
  ...
  getgen9() {
    return this._http
      .get<{}>(`https://pokeapi.co/api/v2/pokemon?limit=105&offset=905`)
      .pipe(
